...
async function updateReviewedMatches() {
  if (!fs.existsSync('programme-review-results.json')) {
    console.error('❌ programme-review-results.json not found!')
    console.log('   Run `npx tsx scripts/review-questionable-matches.ts` first')
    process.exit(1)
  }

  const data = JSON.parse(fs.readFileSync('programme-review-results.json', 'utf-8'))
  const approved = data.approved

  if (approved.length === 0) {
    console.log('ℹ️  No approved matches to update')
    return
  }

  console.log(`🔄 Updating ${approved.length} approved matches...\n`)
